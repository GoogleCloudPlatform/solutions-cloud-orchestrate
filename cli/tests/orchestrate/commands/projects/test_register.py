...
import orchestrate.commands.projects.register
...
def run(execute_command_with_options, arguments):
  ...
  command = orchestrate.commands.projects.register.Command()
  command.run(options, arguments)


@mock.patch('orchestrate.commands.projects.register.Command.configure')
@mock.patch('orchestrate.base.command.OrchestrateCommand.execute')
@mock.patch('orchestrate.main.execute_command_with_options')
def test_no_execution(execute_command_with_options, execute, configure):
  ...
  with pytest.raises(SystemExit):
    run(execute_command_with_options, ['projects', 'register', '--help'])
  assert execute_command_with_options.call_count == 1
  assert configure.call_count == 0
  assert execute.call_count == 0
  ...
  with pytest.raises(SystemExit):
    run(execute_command_with_options,
        ['projects', 'register', '--non-existent-option'])
  assert execute_command_with_options.call_count == 1
  assert configure.call_count == 0
  assert execute.call_count == 0
...
def test_execution_with_defaults(
    execute_command_with_options, execute, configure):
